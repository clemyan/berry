...
The `$npm_execpath` environment variable will get replaced by the right binary depending on the package manager your consumers will use. Yarn also supports just calling :commandline[`run <name>`] without any mention of the package manager, but to this date no other package manager does.
